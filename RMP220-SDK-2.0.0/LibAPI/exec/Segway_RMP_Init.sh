...
echo -e "\nFile Version v1.0.0, 16 Nov 2023 edited.\n"
...
if [ $commu_type == "UART" -o $commu_type == "CAN" ]; then
	echo -e "Communication type is $commu_type.\n"
else
	echo -e "Commu_type variable error, please check the script!!!"
	exit 0
fi

cmd_uart() {

	install_result_1="yes"
	if [ $install_result_1 == "no" ]; then
		echo -e "Install software stty first,please!\n"
		exit 0
	elif [ $install_result_1 == "yes" ]; then
		echo -e
	else
		echo -e "install_result_1 variable error, please check the script!!!\n"
		exit 0
	fi
...
cmd_can() {
	...
	install_result_2="yes"
	if [ $install_result_2 == "no" ]; then
		echo -e "Install software busybox and modprobe first,please!\n"
		sudo apt install busybox -y
		sudo apt install modprobe -y
		exit 0
	elif [ $install_result_2 == "yes" ]; then
		echo -e
	else
		echo -e "install_result_2 variable error, please check the script!!!\n"
		exit 0
	fi

	echo -e "\nSegway-Ninebot notice: CAN settings for the Nvidia Jetson platform,you can also costomize changes.\n"
